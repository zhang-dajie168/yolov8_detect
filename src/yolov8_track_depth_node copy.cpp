...
private:
    ...
    // 获取深度图像中某点的深度值
    float get_depth_at_point(float x,float y)
    {   
        std::lock_guard<std::mutex> lock(depth_mutex_);
        // 你可以根据相机的内参进行深度值的转换
        if (depth_image_.empty())
        {
            std::cout<<"depth_image_ is empty!!!"<<std::endl;
            return 0.0f;  // 如果没有深度图像返回0
        }

        float depth_value = depth_image_.at<float>(y, x)/1000.0;
    ...
    void publish_decobj_to_trackobj(object_detect_result_list &results, std::vector<Object> &trackobj)
    {
    	...
    	for (int i = 0; i < results.count; i++) {
            Object obj;
            if(results.results[i].cls_id != 0)continue;
    ...
    void draw_tracking_results(cv::Mat &img, const std::vector<STrack> &tracks)
    {
        ...
        for (const auto &track : tracks)
        {
            ...
            // 从深度图像中获取中心点的深度值
            float depth = get_depth_at_point(center_x,center_y);
